...
export function DropdownPanel({ items, isOpen, isActive, onClose, viewAllHref, viewAllLabel }: DropdownPanelProps) {
  ...
  const showViewAll = viewAllHref && items.length > 5;

  return (
    <div
      className={`absolute left-1/2 top-full -translate-x-1/2 transition-opacity duration-150 ${isOpen ? "pointer-events-auto opacity-100" : "pointer-events-none opacity-0"}`}
    >
      <div className="w-68 rounded-2xl border border-neutral-200 bg-white p-3 shadow-xl">
